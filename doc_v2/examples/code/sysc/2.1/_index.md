...
如果你是軟體工程師，可以把 SystemC 2.1 想像成一次重大的**框架升級** -- 就像 Java 從 1.4 升級到 5.0 時新增了 generics、concurrent utilities 一樣，SystemC 2.1 新增了動態 process、fork-join、export 機制等等。
...
| `sc_spawn` (動態 process) | 動態建立 thread (`new Thread()` / `go func()`) | 在模擬執行期間動態建立新的執行單元 |
| `SC_FORK` / `SC_JOIN` | `Promise.all()` / `CompletableFuture.allOf()` | 平行派發多個任務，等待全部完成再繼續 |
| `reset_signal_is` | Graceful restart / Circuit breaker pattern | 當 reset 信號觸發時，thread 自動回到起始狀態 |
| `sc_report` | Logging framework (log4j, slog, Python logging) | 統一的訊息報告機制，可依嚴重等級和 ID 過濾 |
| `scx_barrier` | `CyclicBarrier` (Java) / `sync.WaitGroup` (Go) | 多個 thread 在同一個同步點等待，全到齊後一起放行 |
| `scx_mutex_w_policy` | Mutex with fairness policy (`ReentrantLock(true)`) | 支援 FIFO 或 RANDOM 仲裁策略的互斥鎖 |
...
| `forkjoin/` | Fork-join 平行執行與動態 process 建立 | `Promise.all()` | [forkjoin.md](forkjoin.md) |
...
| `scx_barrier/` | Barrier 同步原語，多 thread 等待全到齊 | `CyclicBarrier` | [scx-barrier.md](scx-barrier.md) |
| `scx_mutex_w_policy/` | 支援 FIFO / RANDOM 仲裁策略的互斥鎖 | `ReentrantLock` with fairness | [scx-mutex-w-policy.md](scx-mutex-w-policy.md) |
...
| `sc_spawn()` | `go func()` / `new Thread(runnable)` | 在模擬執行期間動態建立 SC_THREAD 或 SC_METHOD |
| `SC_FORK` / `SC_JOIN` | `Promise.all([p1, p2, p3])` | 用 macro 包裝多個 `sc_spawn`，等待所有 process 結束 |
| `reset_signal_is()` | 註冊 shutdown hook / circuit breaker trigger | 當指定信號為特定值時，thread 自動重新從頭執行 |
| `sc_report_handler` | `log4j.Logger` / `slog.Handler` | 設定不同 message ID 和嚴重等級的處理動作 |
| `scx_barrier` | `java.util.concurrent.CyclicBarrier` | 計數器歸零時，所有等待的 thread 同時釋放 |
| `sc_mutex` + policy | `java.util.concurrent.locks.ReentrantLock(fair)` | 在多 process 競爭時，依策略決定誰先取得鎖 |
